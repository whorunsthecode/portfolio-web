...
function SundrySignage() {
  // Three pieces of signage that read the shop as a 士多:
  //   1. Big red horizontal "士多" signboard above the gate, facing
  //      into the alley
  //   2. Coca-Cola decal on the fridge glass door
  //   3. Vintage wall calendar (1985 美女月曆) on the back wall
  //   4. "煙仔" tobacco strip across the cigarette shelf
  ...
  const signboardTex = useMemo(() => {
    ...
    // Red painted board with brushed-paint texture
    ctx.fillStyle = '#a01818'
    ctx.fillRect(0, 0, 512, 128)
    // Slight darker patches for paint unevenness
    for (let i = 0; i < 30; i++) {
    ...
    // Big yellow "士多" centred, with smaller "SUNDRY" underneath
  ...
  const cokeTex = useMemo(() => {
    ...
    // Big red disc/oval background
    ctx.fillStyle = '#c81818'
    ctx.beginPath()
    ctx.ellipse(128, 128, 110, 90, 0, 0, Math.PI * 2)
    ctx.fill()
    // White swooshy "Coca-Cola" approximation — 2 horizontal stripes
  ...
  const calendarTex = useMemo(() => {
    ...
    // Top red strip — "1985 美女月曆" header
    ctx.fillStyle = '#c81818'
    ...
    // Pin-up girl silhouette (dark blob suggesting figure)
    ctx.fillStyle = '#3a2a20'
    ctx.beginPath()
    ctx.ellipse(128, 160, 60, 70, 0, 0, Math.PI * 2)  // head/torso
    ctx.fill()
    ctx.beginPath()
    ctx.ellipse(128, 240, 50, 50, 0, 0, Math.PI * 2)  // lower
    ctx.fill()
    // Grid of dates at bottom — small marks suggesting a calendar grid
    ...
    ctx.fillStyle = '#a01818'
    ctx.fillRect(20, 320, 24, 8) // Sunday red
    return new THREE.CanvasTexture(c)
  }, [])

  // === 4. 煙仔 tobacco strip texture for cigarette shelf ===
  ...
  return (
    <group>
      {/* === Horizontal 士多 signboard above the gate, attached to the
          alley left wall, facing into the alley (+X normal) === */}
      <mesh
      ...
      >
        ...
        <meshStandardMaterial map={signboardTex} side={THREE.DoubleSide}
          emissive={'#a01818'} emissiveIntensity={0.15} />
      </mesh>
      {/* Mounting bracket / shadow box behind sign */}
      <mesh
      ...
      {/* === Coca-Cola decal on fridge glass door ===
          Fridge centre is at (counterCenterX + 0.45, 0.45, counterZ)
          where counterCenterX = (-0.9 + -2.4) / 2 = -1.65 → fridge x = -1.2
          Fridge glass face is at z = counterZ + 0.21 = -7.8 + 0.21 = -7.59
          Decal sits in front of glass facing +Z. */}
      ...
      {/* === 煙仔 strip across the cigarette shelf (shelf 3 at y≈1.5) ===
          The strip sits ON the shelf above the cigarette boxes, like a
          retail tobacco header banner. */}
      <mesh
        position={[SHOP_X_BACK + 0.27 + 0.02, 1.5 + 0.14, SHOP_Z_MID]}
        rotation={[0, Math.PI / 2, 0]}
      >
